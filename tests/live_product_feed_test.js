...
assert('worker common immunity template is detailed enough for top matches plus additional options', /ingredientTemplates/.test(worker) && /immune:\s*\[/.test(worker) && /Butyrate \/ postbiotics/.test(worker) && /Prebiotic fiber/.test(worker) && /Multivitamin support/.test(worker) && /COMMON_RESPONSE_CACHE_PREFIX\s*=\s*'nutrapass:common-response:v5:'/.test(worker));
