...
assert('frontend excludes non-ingestible body-care products from nutrition result rails', /npIsNonIngestibleProduct/.test(html) && /conditioner\|shampoo\|sun balm/.test(html) && /filter\(function\(p\)\{return !npIsNonIngestibleProduct\(p,goal\);\}\)/.test(html));
...
assert('worker common immunity template is detailed enough for top matches plus additional options', /ingredientTemplates/.test(worker) && /immune:\s*\[/.test(worker) && /Butyrate \/ postbiotics/.test(worker) && /Prebiotic fiber/.test(worker) && /Multivitamin support/.test(worker) && /COMMON_RESPONSE_CACHE_PREFIX\s*=\s*'nutrapass:common-response:v2:'/.test(worker));
