...
assert('follow-up panel preserves the results above state instead of replacing it', /function npReportContextForFollowup/.test(html) && /window\.npLastReport/.test(html) && /report:npReportContextForFollowup\(data\)/.test(html));
...
assert('follow-up answers can show one gap-filling question and right-side related products', /np-follow-gaps/.test(html) && /slice\(0,1\)/.test(html) && /np-follow-msg\.has-products/.test(html) && /aria-label="Related products from these results"/.test(html) && /npProductCards\(productItems\)/.test(html));
...
assert('follow-up prompt requests warmer tighter responses', /warm, kind, and helpful/.test(worker) && /55–110 words/.test(worker) && /Avoid wall-of-text responses/.test(worker) && /the results above/.test(worker) && /Avoid stiff phrases like "your report"/.test(worker));
...
assert('worker follow-up fallback preserves context language', /staticFollowUpFallback/.test(worker) && /results above/.test(worker));
